...
command=("./software/freebayes" "$@")
(
#$command | head -100 | grep "^#" # generate header
# iterate over regions using gnu parallel to dispatch jobs
cat "$regionsfile" | ./software/parallel -k -j "$ncpus" "${command[@]}" --region {}
) | ./software/vcffirstheader \
    | ./software/vcfstreamsort -w 1000 | ./software/vcfuniq # remove duplicates at region edges